...
namespace net_deserializer
{
    ...
    template<> std::unique_ptr<Node> make_primitive<bool>(
    ...
    template<> std::unique_ptr<Node> make_primitive<char>(
        const std::string &name, BinaryReader &r)
    {
        return std::make_unique<LeafNode>(name, r.read<char>());
    }

    template<> std::unique_ptr<Node> make_primitive<int8_t>(
        const std::string &name, BinaryReader &r)
    {
        return std::make_unique<LeafNode>(name, r.read<int8_t>());
    }

    template<> std::unique_ptr<Node> make_primitive<int16_t>(
        const std::string &name, BinaryReader &r)
    {
        return std::make_unique<LeafNode>(name, r.read<int16_t>());
    }

    template<> std::unique_ptr<Node> make_primitive<int32_t>(
        const std::string &name, BinaryReader &r)
    {
        return std::make_unique<LeafNode>(name, r.read<int32_t>());
    }

    template<> std::unique_ptr<Node> make_primitive<int64_t>(
        const std::string &name, BinaryReader &r)
    {
        return std::make_unique<LeafNode>(name, r.read<int64_t>());
    }

    template<> std::unique_ptr<Node> make_primitive<uint8_t>(
        const std::string &name, BinaryReader &r)
    {
        return std::make_unique<LeafNode>(name, r.read<uint8_t>());
    }

    template<> std::unique_ptr<Node> make_primitive<uint16_t>(
        const std::string &name, BinaryReader &r)
    {
        return std::make_unique<LeafNode>(name, r.read<uint16_t>());
    }

    template<> std::unique_ptr<Node> make_primitive<uint32_t>(
        const std::string &name, BinaryReader &r)
    {
        return std::make_unique<LeafNode>(name, r.read<uint32_t>());
    }

    template<> std::unique_ptr<Node> make_primitive<uint64_t>(
        const std::string &name, BinaryReader &r)
    {
        return std::make_unique<LeafNode>(name, r.read<uint64_t>());
    }

    template<> std::unique_ptr<Node> make_primitive<float>(
        const std::string &name, BinaryReader &r)
    {
        return std::make_unique<LeafNode>(name, r.read<float>());
    }

    template<> std::unique_ptr<Node> make_primitive<double>(
        const std::string &name, BinaryReader &r)
    {
